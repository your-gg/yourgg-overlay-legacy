...
//!
//! Why a separate exe: when `SetWindowsHookExW` is called from a large host
//! process (Electron/Chromium), Vanguard validation makes it block ~11-16s;
//! from this small dedicated exe it is ~2s (measured 2026-06-16). The helper
//! must be the SAME architecture as the target process.
//!
//! Usage:  injector-helper <pid> <dll-path>
//! Exit:   0 = hook installed and the overlay's IPC server is up
//!         1 = bad args / install failed / timed out
...
/// Upper bound for both the hook install and the IPC-server wait.
...
async fn main() -> ExitCode {
    match run().await {
        Ok(()) => {
            eprintln!("[injector-helper] OK: hook installed, overlay IPC server up");
            ExitCode::SUCCESS
        }
        Err(err) => {
...
async fn run() -> anyhow::Result<()> {
    ...
    eprintln!(
        "[injector-helper] pid={pid} dll={} (helper pid {})",
        dll_path.display(),
        std::process::id()
    );
