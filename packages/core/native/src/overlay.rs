...
///
/// Installing the hook inside this (Electron/Chromium) process makes Vanguard
/// block ~11-16s on `SetWindowsHookExW`; doing it from the dedicated helper exe
/// is ~2s (measured). The DLL pins itself on first hook fire, so it survives the
/// helper exiting (which removes the hook); we then connect from here.
///
...
async fn attach_via_helper(
    ...
    // this process. Capture stderr so the helper's diagnostics / failure cause
    // reach us; kill_on_drop so a cancelled/timed-out attach can't leak it.
    ...
    // Surface the helper's diagnostics (phase A / A1-A3 timings) on our stderr.
